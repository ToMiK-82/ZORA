"""
Агент-закупщик для прогнозирования остатков и формирования заказов.
...
class Purchaser(BaseAgent):
    ...
    def _process_specific(self, query: str, context: str) -> Dict[str, Any]:
        """
        Обрабатывает запросы по закупкам с использованием LLM.
...
Пожалуйста, предоставь анализ по закупкам, прогнозы остатков и рекомендации по заказам.
...
"""
        ...
        try:
            ...
            return {
                "success": False,
                "result": f"Ошибка при обработке запроса закупщиком: {str(e)}",
                "agent": self.agent_name,
                "context_used": bool(context),
                "error": str(e)
            }
